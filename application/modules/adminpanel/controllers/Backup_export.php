...
class Backup_export extends Admin_Controller {
    ...
    public function export_members() {
        ...
        if ($this->zip->archive($zip_path .'.zip')) {
            $this->session->set_flashdata('success', '<p>'. $this->lang->line('export_members_success') .'</p>');
        }else{
            $this->session->set_flashdata('error', '<p>'. $this->lang->line('export_members_failed') .'</p>');
        }
        ...
        if ($this->input->post('email_memberlist') != "") {
            ...
            if ($this->email->send())
            {
                $this->session->set_flashdata('success', '<p>'. $this->lang->line('export_members_success_send') .'</p>');
            }else{
                $this->session->set_flashdata('error', '<p>'. $this->lang->line('export_members_failed_send') .'</p>');
            }
            $this->email->clear();
        }
    ...
    public function export_database() {
        ...
        if ($this->zip->archive($path .".zip")) {
            $this->session->set_flashdata('success', '<p>'. $this->lang->line('export_database_success') .'</p>');
        }else{
            $this->session->set_flashdata('error', '<p>'. $this->lang->line('export_database_failed') .'</p>');
        }
        ...
        if ($this->input->post('email_db') != "") {
            ...
            if ($this->email->send())
            {
                $this->session->set_flashdata('success', '<p>'. $this->lang->line('export_database_success_send') .'</p>');
            }else{
                $this->session->set_flashdata('error', '<p>'. $this->lang->line('export_database_failed_send') .'</p>');
            }
            $this->email->clear();
        }
