...
class List_Members_model extends CI_Model {
    ...
    public function toggle_ban($user_id, $banned) {
        ...
        if ($data['banned'] == true) {
            $this->db->select('ip_address')->from('user_cookie_part')->where('user_id', $user_id);
            $q = $this->db->get();

            $this->db->where('ip_address', $q->row()->ip_address)->delete(DB_PREFIX .'ci_sessions');
        }
    ...
    public function ban_checked($ids) {
        $this->db->where_in('user_id', $ids)->where('username != ', Settings_model::$db_config['root_admin_username'])->update(DB_PREFIX .'user', array('banned' => true));
        return $this->db->affected_rows();
    }

    public function unban_checked($ids) {
        $this->db->where_in('user_id', $ids)->where('username != ', Settings_model::$db_config['root_admin_username'])->update(DB_PREFIX .'user', array('banned' => false));
        return $this->db->affected_rows();
    }

    public function activate_checked($ids) {
        $this->db->where_in('user_id', $ids)->where('username != ', Settings_model::$db_config['root_admin_username'])->update(DB_PREFIX .'user', array('active' => true));
        return $this->db->affected_rows();
    }

    public function deactivate_checked($ids) {
        $this->db->where_in('user_id', $ids)->where('username != ', Settings_model::$db_config['root_admin_username'])->update(DB_PREFIX .'user', array('active' => false));
        return $this->db->affected_rows();
    }
